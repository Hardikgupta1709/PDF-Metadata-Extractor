...
GOOGLE_DRIVE_ENABLED = True
GOOGLE_DRIVE_FOLDER = "Research Paper Submissions"
GOOGLE_DRIVE_FOLDER_ID = os.getenv("GOOGLE_DRIVE_FOLDER_ID", "1f1YT5McP0w6Ub4rl5MT1MgjTX7s7j21j")
SUBMISSIONS_FOLDER = "submitted_papers"
SUBMISSIONS_FILE = "submissions.csv"
...
def get_gcp_credentials():
    """Get GCP credentials from environment variables or secrets"""
    try:
...
def get_drive_service():
    """Get Drive service using service account"""
    try:
        from google.oauth2.service_account import Credentials
        from googleapiclient.discovery import build
...
with st.sidebar:
    ...
    if not st.session_state.admin_authenticated:
        ...
        if "✅" in status:
            ...
            if folder_link and st.button("📁 Open Folder", use_container_width=True):
                st.markdown(f"[Open Folder]({folder_link})")
        else:
            st.error(status)
            st.info("Check environment variables configuration")
...
if st.session_state.show_success:
    st.success("🎉 Submission successful!")
    st.info("✅ All details saved. Admin will review and contact you.")
